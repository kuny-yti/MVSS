#ifndef BS_NUMBER_DOUBLE
#define BS_NUMBER_DOUBLE
...
static char *_dtoa_( double d, int mode, int ndigits, int *decpt,
...
char* double_string(double d, int mode, int ndigits, int *decpt,
                              int *sign, char **rve, char **resultp)
{
...
#    endif
    ...
    char *s = _dtoa_(d, mode, ndigits, decpt, sign, rve, resultp);
...
double string_double(const char *s00, char const **se)
{
...
#else
#include <errno.h>
inline char *double_string( double d, int mode, int ndigits, int *decpt,
                    int *sign, char **rve, char **resultp, int *slen)
{
    if(rve)
      *rve = 0;
    ...
    *resultp = static_cast<char*>(bsMalloc(n + 1));
    strncpy(*resultp, res, n + 1);
    if (slen)
        *slen = n;
    return *resultp;
}

inline double string_double(const char *s00, const char **se)
{
    return strtod((char*)s00, (char**)se);
}
#endif
#endif // BS_NUMBER_DOUBLE
