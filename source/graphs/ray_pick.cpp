...
ray_pick::ray_pick(const fpoint3 &rayStart, const fpoint3 &rayEnd)
{
    start = rayStart;
    end = rayEnd;
}

fpoint3 ray_pick::Normalise(const fpoint3 &v) const
{
    fpoint3 out;
...
fpoint3 ray_pick::delivery(const fpoint3 &U, const fpoint3 &V)const
{
    fpoint3 ret;
...
fpoint3 ray_pick::CalcPlaneNormal(const fpoint3 &P1,const fpoint3 &P2, const fpoint3 &P3)const
{
   fpoint3 V1, V2;
   fpoint3 result;
...
ray_pick::LineEqv::LineEqv(const fpoint3 &m0, const fpoint3 &s)
{
    //默认1、3方程
    a1 = s.y;
    b1 = s.z - s.x;
...
void ray_pick::LineEqv::TestPoint(const fpoint3 &p, double &res1, double &res2)
{
...
bool ray_pick::IsInTrg(const fpoint3 &vtxA, const fpoint3 &vtxB, const fpoint3 &vtxC,
                       const TriEqaResult &pHit)
{
    ...
    fpoint3 pos = fpoint3(pHit.x, pHit.y, pHit.z);
...
bool ray_pick::intersect(const fpoint3 &vtxA, const fpoint3 &vtxB, const fpoint3 &vtxC,
                         fpoint3 &pos)
{
    PlaneEqv curFace = PlaneEqv(vtxA, CalcPlaneNormal(vtxA, vtxB, vtxC));
    fpoint3 ori = end - start;
    ...
    pos = fpoint3((float)pHit.x, (float)pHit.y, (float)pHit.z);

    fpoint3 tr = (pos - start) * ori;
...
bool ray_pick::pick (fpoint3 &pos, const fpoint3 &rayStart, const fpoint3 &rayEnd,
                     const fsize rect)
{
    fpoint3 vtxA1, vtxB1, vtxC1;
    fpoint3 vtxA2, vtxB2, vtxC2;
    fpoint3 vtxA3, vtxB3, vtxC3;
    float sw = (float)rect.width/2.;
    float sh = (float)rect.height/2.;

    vtxA1 = fpoint3(pos.x -sw, pos.y -sh, pos.z);
    vtxB1 = fpoint3(pos.x +sw, pos.y -sh, pos.z);
    vtxC1 = fpoint3(pos.x , pos.y +sh, pos.z);

    vtxA2 = fpoint3(pos.x , pos.y -sh, pos.z -sh);
    vtxB2 = fpoint3(pos.x , pos.y -sh, pos.z +sh);
    vtxC2 = fpoint3(pos.x , pos.y +sh, pos.z );

    vtxA3 = fpoint3(pos.x -sh, pos.y, pos.z -sh);
    vtxB3 = fpoint3(pos.x -sh, pos.y, pos.z +sh);
    vtxC3 = fpoint3(pos.x +sh, pos.y, pos.z );
